...
class QueryHandler{
    ...
    static public function updateByColumnsAndValues($table , $ColumnsAndNewValuesForUpdatingProccess_array , $ColumnsAndValuesForCondition_array = array() , $operators_array = array("=")){
       
            $updating_query_string = "update " . $table . " set ";
            $i = 0;
            $countOfColumnsToUpdate = count($ColumnsAndNewValuesForUpdatingProccess_array);
            foreach($ColumnsAndNewValuesForUpdatingProccess_array as $column => $value){
                $value = gettype($value) == 'string' ? "'" . $value . "'" : $value;
                if($i == $countOfColumnsToUpdate - 1){
                    $updating_query_string .= $column . " = " . $value;
                }else{
                    $updating_query_string .= $column . " = " . $value . " , ";
                }
                $i++;
            ...
            $countOfColumnsToCheckInCondition = count($ColumnsAndValuesForCondition_array);
            $updating_query_string .= $countOfColumnsToCheckInCondition > 0 ? " where " : "";
            $countOfOperators = count($operators_array);
            $m = 0;
            foreach($ColumnsAndValuesForCondition_array as $ConditionColumn => $value){
                $value = gettype($value) == 'string' ? "'" . $value . "'" : $value;
                if($m == $countOfColumnsToCheckInCondition - 1){
                    if($countOfOperators < $countOfColumnsToCheckInCondition){
                        $updating_query_string .= $ConditionColumn . " " . $operators_array[0] . " " . $value;
                    }else{
                        $updating_query_string .= $ConditionColumn . " " . $operators_array[$m] . " " . $value;
                    }
                }else{
                    if($countOfOperators < $countOfColumnsToCheckInCondition){
                        $updating_query_string .= $ConditionColumn . " " . $operators_array[0] . " " . $value . " and ";
                    }else{
                        $updating_query_string .= $ConditionColumn . " " . $operators_array[$m] . " " .  $value . " and ";
                    }
                }
                $m++;
            }
            return self::updateByqueryStatment($updating_query_string);
